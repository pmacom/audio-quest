...
// Utility: 3D rotation around the Y axis
mat3 rotate3D(float angle, float s) {
    float c = cos(angle);
    return mat3(
        c, 0.0, s,
        0.0, 1.0, 0.0,
...
void main() {
    ...
    vec3 color = vec3(0.0);
    float mainLoopCount = TWEAK_1 * 40.0; // Main loop count (max 40)
    float posNormScale = TWEAK_2 * 1.0;    // Position normalization scaling
    float zIncrement = TWEAK_3 * 0.5;      // Z increment per loop
    float trigFreq = TWEAK_4 * 1.0;        // Trig frequency scaling
    float outDivisor = TWEAK_9 * 2000.0;   // Output normalization divisor
    float nonlinearity = TWEAK_10 * 1.0;   // Output nonlinearity
    // Per-channel phase offsets for color richness
    float phaseR = TWEAK_6 * 0.0;
    float phaseG = TWEAK_7 * 2.0;
    float phaseB = TWEAK_8 * 4.0;
    
    // Main color accumulation loop
    for(float i = 0.0, z = 0.0; i < 40.0; i++) {
        if(i >= mainLoopCount) break;
        // Position calculation
        vec3 p = z * normalize(FC.rgb * 2.0 - r.xyy) * posNormScale;
        p.z -= t;
        // 3D rotation for dynamic effect
        mat3 rot = rotate3D(t, sin(p.x + p.y + p.z));
        vec3 v = ceil(p / 0.1) * 0.1;
        v = abs(cos(v) * rot) - 0.7 + z / 20.0;
        float d = length(max(v, v.yzx * 0.3)) * zIncrement;
        z += d;
        // Per-channel color accumulation
        color.r += (cos(z / 0.5 + phaseR) + 1.1) / d;
        color.g += (cos(z / 0.5 + phaseG) + 1.1) / d;
        color.b += (cos(z / 0.5 + phaseB) + 1.1) / d;
    }
...
export const FREQ530_AMAZING_SHADER_AI_V6: ShaderSettings = {
    shaderId: "amazing-shader-ai-v6",
    isActive: true,
    name: "Amazing AI Shader 6 (Expanded & Colorful)",
