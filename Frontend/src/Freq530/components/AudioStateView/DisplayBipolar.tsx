...
const DisplayBipolar = ({ label, value, color, className = "" }: DisplayBipolarProps) => {
  const widthPercentage = Math.abs(value) * 50;
  const isNegative = value < 0;
  const position = isNegative ? "right" : "left";
  const wrapperClasses = useMemo(() => cn("w-full h-full mb-1", className), [className]);
  const barClasses = useMemo(() => cn("absolute top-0 bottom-0 h-full"), []);

  return (
    <div className={wrapperClasses} style={getColorStyles(color)}>
      <div className="absolute top-0 left-0 w-full h-[1rem] rounded-sm overflow-hidden" aria-hidden="true">
