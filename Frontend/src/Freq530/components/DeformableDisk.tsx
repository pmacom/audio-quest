...
export function DeformableDisk({
  ...
  useFrame(() => {
    ...
    if (!meshRef.current || !initialPositions) {
      if (debug && debugCounterRef.current % 60 === 0) {
        console.log('❌ NO MESH OR INITIAL POSITIONS (DISK)'); // debug
      }
      return;
    }
    ...
    if (!previousPositions.current) {
      ...
      if (debug) console.log('🔧 INITIALIZED DISK PREVIOUS POSITIONS:', positions.count, 'vertices'); // debug
    }
    
    if (!previousColors.current && useVertexColors) {
      previousColors.current = new Float32Array(colors.count * 3);
      if (debug) console.log('🎨 INITIALIZED DISK VERTEX COLORS:', colors.count, 'vertices'); // debug
    }
    ...
    if (!frequencyGridMap || frequencyGridMap.length === 0) {
      ...
      if (debug && debugCounterRef.current % 120 === 0) {
        console.log('🔇 NO FREQUENCY DATA: Disk returning to flat state'); // debug
      }
      return;
    }
    ...
    if (debugCounterRef.current % 60 === 0 && hasValidData) {
      ...
      if (debug) console.log('🎵 DISK FREQUENCY DATA (VINYL):', { // debug
