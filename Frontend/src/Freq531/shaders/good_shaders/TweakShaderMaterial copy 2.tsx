...
export const TweakShaderMaterial = ({ shader }: TweakShaderMaterialProps) => {
  ...
  useFrame(({ clock }) => {
    if (materialRef.current) {
      ...
      // // ZOOM
      materialRef.current.uniforms.TWEAK_2.value = (audioState.amplitudeDynamic / 30) + .5
      ...
      materialRef.current.uniforms.TWEAK_10.value = (audioState.lowDynamic) + 2
      materialRef.current.uniforms.TWEAK_5.value = (audioState.snareDynamic / 10) + .5
      materialRef.current.uniforms.TWEAK_6.value = (audioState.cosNormal)
    }
  });
