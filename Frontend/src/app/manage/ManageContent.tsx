...
import type { Tag, Directory, VideoFile } from "@/Freq530/video-manager/types"
...
export function ManageContent({ section }: Props) {
    ...
    async function load() {
      ...
      try {
        ...
        const vids: VideoFile[] = data.videos.map((v) => {
          ...
          const thumb = v.path
            .replace(/^\/videos\//, "/thumbnails/videos/")
            .replace(/\.[^/.]+$/, ".jpg")
          return {
            id: v.path,
            name: v.name,
        ...
        setVideos(vids)
        setDirectories((prev) =>
          prev.map((d) => (d.id === "1" ? { ...d, videoCount: vids.length } : d))
        )
      } catch {
        // ignore errors
      }
  ...
  const deleteTag = (id: string) => {
    ...
    if (tag) {
      setVideos((prev) =>
        prev.map((video) => ({ ...video, tags: video.tags.filter((t) => t !== tag.name) }))
      )
